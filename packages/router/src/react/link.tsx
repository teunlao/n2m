...
const LinkView = <Params extends RouteParams>(props: LinkProps<Params>, ref: ForwardedRef<HTMLAnchorElement>) => {
  const { to, params, query, activeClassName, inactiveClassName, ...linkProps } = props
  if (typeof props.to === 'string') {
    return <NormalLink ref={ref} href={props.to} {...linkProps} className={clsx(props.className)} />
  }
  // @ts-expect-error
  return <RouteLink ref={ref} {...props} />
}

const NormalLink = forwardRef<HTMLAnchorElement, AnchorHTMLAttributes<HTMLAnchorElement>>((props, ref) => {
...
const RouteLinkView = <Params extends RouteParams>(
  ...
  const { to, params, query, className, activeClassName, inactiveClassName, onClick, children, target, ...linkProps } =
    props
...
export const RouterLink = forwardRef(LinkView) as <Params extends RouteParams>(
