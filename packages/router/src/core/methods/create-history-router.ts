...
export function createHistoryRouter({
  ...
  const $firstActiveRoute = $activeRoutes.map((activeRoutes) => activeRoutes?.[0], { skipVoid: false })
  ...
  const historyUpdatedParsed = sample({
    clock: [historyUpdated, subscribeHistoryFx.done],
    source: $history,
    filter: Boolean,
    fn: (history) => ({
      path: history.location.pathname,
      query:
        serialize?.read(history.location.search) ?? Object.fromEntries(new URLSearchParams(history.location.search)),
    }),
  })
  ...
  for (const routeObj of remappedRoutes) {
    ...
    const routeStateChangeRequested = {
      ...
      initialized: sample({
        ...
        filter: combine(routeObj.route.$isOpened, $hydrateMode, (isOpened, hydrateMode) => isOpened && !hydrateMode),
      }),
      opened: sample({
    ...
    sample({
      ...
      filter: ([params, query], next) => !paramsEqual(params, next.params) || !paramsEqual(query, next.query),
  ...
  $activeRoutes.on(recalculated, (_, { matching }) => matching.map((recheckResult) => recheckResult.routeObj.route))
  ...
  sample({
    ...
    filter: ({ localQuery, isNavigateInProgress, realHistory }, query) => {
      ...
      return isNavigateInProgress || !paramsEqual(query, realQuery) || !paramsEqual(localQuery, query)
    },
    fn({ path }, query) {
...
const containsCurrentRoute = (routeObj: RouteObject<any>) => (recheckResults: RecalculationResult<any>[]) => {
  const recheck = recheckResults.find((recheckResult) => recheckResult.routeObj.route === routeObj.route)
  if (!recheck) {
    return
  }
  return {
