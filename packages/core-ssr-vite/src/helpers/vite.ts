...
export const isCssModulesFile = (assetPath: string) => {
  return ASSET_REGEXES.cssModules.test(assetPath)
}

export const isVueStyle = (assetPath: string) => {
  return ASSET_REGEXES.vueStyles.test(assetPath)
}
...
export const isAssetHandledByVite = (assetPath: string, basePath?: string) => {
  ...
  return (
    ...
    ASSET_REGEXES.vueStyles.test(pathname) ||
