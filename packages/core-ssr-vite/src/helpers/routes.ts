...
export const generateSSRManifest = (clientManifest: ViteClientManifest) => {
...
export const generateEntryManifest = (clientManifest: ViteClientManifest) => {
  const entry = Object.values(clientManifest).find((m) => {
    // while we dynamically inject vue application, we should pass dynamic entries that have .vue extensions TODO remove
    // it's only production issue
    return m.isEntry || m.imports?.some((i) => i.endsWith('.vue')) || m.dynamicImports?.some((i) => i.endsWith('.vue'))
  })
...
const getAssetType = (asset: string): AssetType | null => {
  let ext = asset.split('.').at(-1)?.toLowerCase()
