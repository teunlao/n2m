...
describe('Array field ssr api', () => {
  test.skip('check array field values serialization & deserialization', async () => {
...

    {
      ...
      const mappedValues = scope.getState(field.$values).map((item) => scope.getState(mapSchema(item).$values))
