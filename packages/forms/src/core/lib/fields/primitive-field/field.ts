...
import type { CreatePrimitiveFieldOptions, PrimitiveField, PrimitiveValue } from './types'
import { FieldBatchedPayload, FieldBatchedSetter, FieldError, InnerFieldApi } from '../types'
...
export function createField<T extends PrimitiveValue>(
  ...
  return {
    ...
    fork: (options?: CreatePrimitiveFieldOptions) => createField(defaultValue, { ...overrides, ...options }),
    ...
    '@@unitShape': () => ({
      ...
      changeError,
      clearInnerError: setInnerError.prepend(() => null),
      change,
      reset,
    }),
