...
export function setupPrimitiveField(
  ...
  const changeValueFx = createEffect(({ value }: { value: any; batchInfo?: { id: string } }) => {
  ...
  const resetFx = createEffect(({ value, error }: { value: any; error: FieldError; batchInfo?: { id: string } }) => {
    resultValuesNode[key] = value
    resultErrorsNode[key] = error
  })
