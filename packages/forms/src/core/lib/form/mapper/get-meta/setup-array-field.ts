import { ArrayField, FieldError, InnerArrayFieldApi, isPrimitiveValue } from '../../../fields'
...
export function setupArrayField(
  this: This,
  { field: rawField, path, key, resultErrorsNode, resultValuesNode, map }: Props
) {
  ...
  const mapValues = (values: any[]) => {
    values.map((item, index) => {
      if (!isPrimitiveValue(item)) {
        ...
        map.call(this, item, resultValuesNode[key][index], resultErrorsNode[key].errors[index], [
          ...path,
          key,
          index.toString(),
        ])
  ...
  const changeValuesFx = createEffect(({ values }: { values: any[]; batchInfo?: { id: string } }) => {
  ...
  const changeErrorFx = createEffect(({ error }: { error: FieldError; batchInfo?: { id: string } }) => {
  ...
  const resetFx = createEffect(
    ({ error, values }: { values: any[]; error: FieldError; batchInfo?: { id: string } }) => {
      getArrayFieldApi(this.api, apiKey).clearValuesMemory()
  ...
  // not batched flow
  sample({
    clock: [inOrder([field.setInnerError, field.errorChanged]), inOrder([field.changeError, field.errorChanged])],
    source: field.$error,
    fn: (error) => ({ error }),
  ...
  sample({
    clock: [changeValuesFx.done, changeErrorFx.done, resetFx.done, clearFx.done],
    filter: ({ params }) => !!params.batchInfo,
    fn: ({ params }) => ({
      fieldPath: apiKey,
