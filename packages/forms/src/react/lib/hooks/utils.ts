...
import { ReactArrayFieldApi, ReactFields, ReactPrimitiveFieldApi } from '../types'
...
export function getFields<T extends ReadyFieldsGroupSchema>(fields: T, scope: Scope | null): ReactFields<T> {
  const node: any = {}

  const getStoreValue = <T>($store: Store<T>) => getStoreValueByScope($store, scope)

  const bindEvent = <T>(event: EventCallable<T>) => bindEvenByScope(event, scope)
  ...
  for (const fieldName in fields) {
    ...
    switch (field.type) {
      case arrayFieldSymbol: {
        node[fieldName] = {
          values: getStoreValue(field.$values).map((item) => (isPrimitiveValue(item) ? item : getFields(item, scope))),
      ...
      default: {
        node[fieldName] = getFields(fields[fieldName] as ReadyFieldsGroupSchema, scope)
      }
    }
  }
